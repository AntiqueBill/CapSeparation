...
for num_sample=1:N_sample    
    ...
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode6(num_sample,:)=[6,yr];     
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode7(num_sample,:)=[7,yr];     

    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db'); 
    yr=mapminmax(yr);
    mode8(num_sample,:)=[8,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode9(num_sample,:)=[9,yr]; 
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode10(num_sample,:)=[10,yr];   
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    ...
    y=qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode13(num_sample,:)=[13,yr]; 

    
    y=qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) +  LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode14(num_sample,:)=[14,yr]; 
    
    y=qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    ...
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode17(num_sample,:)=[17,yr];    
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) +  LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode18(num_sample,:)=[18,yr]; 
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode19(num_sample,:)=[19,yr]; 
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) +  LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode20(num_sample,:)=[20,yr]; 

    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) +  LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode21(num_sample,:)=[21,yr];
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode22(num_sample,:)=[22,yr];
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode23(num_sample,:)=[23,yr];
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode24(num_sample,:)=[24,yr];
    
    y=qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode25(num_sample,:)=[25,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode26(num_sample,:)=[26,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode27(num_sample,:)=[27,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode28(num_sample,:)=[28,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode29(num_sample,:)=[29,yr];
    
    y=fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
    mode30(num_sample,:)=[30,yr];
    
    y=ask2(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + fsk2(N_code,(3+2*rand(1))*1e6,fs,fd,freqsep,Ac) + qpsk(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + qam16(N_code,(3+2*rand(1))*1e6,fs,fd,Ac) + LFM(N_code,fs,fd,Ac);
    yr=awgn(y,snr,'measured','db');
    yr=mapminmax(yr);
...
save(strcat('../train/',fdata),'train_x','train_y','test_x','test_y','-v7.3')
