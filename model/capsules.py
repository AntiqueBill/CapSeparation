...
class PrimaryCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        a = self.a(x)
        a = self.sigmoid(a)
        out = torch.cat([p, a], dim=1)#按照维度拼接，1是列
...
class ConvCaps(nn.Module):
    ...
    Args:
        B: input number of types of capsules 输入胶囊的个数
        C: output number on types of capsules 输出胶囊的个数
        K: kernel size of convolution
        P: size of pose matrix is P*P
        stride: stride of convolution
    ...
    def __init__(self, B=32, C=32, K=3, P=4, stride=2, iters=3,
        ...
        self.weights = nn.Parameter(torch.randn(1, K*K*B, C, P, P))#randn为正态分布，rand为均匀分布
        # op
        self.sigmoid = nn.Sigmoid()
